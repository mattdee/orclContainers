...
select 'Oracle Version: '||version_full|| ' Version' "System Info" from PRODUCT_COMPONENT_VERSION
union all
select 'Connected to: '||name||' Database' "Hello" from V$DATABASE
union all
select 'On host: '||host_name||' Host' "Host Details" from V$INSTANCE;
